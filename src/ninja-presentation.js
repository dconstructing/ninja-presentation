(function(win, doc) {
    ...
    ninjaPresentation.hideAllFragmentsOnPresentation = function () {
        var iterator,
            fragments = doc.getElementsByClassName('fragment');
    ...
    ninjaPresentation.hasSlideOnLocationHash = function () {
        return !!win.location.hash.substring(win.location.hash.indexOf('#'));
    };

    ninjaPresentation.focusSlide = function (slideIdentifier) {
        doc.getElementById(slideIdentifier).style.opacity = 1;
        win.location.hash = slideIdentifier
    };

    ninjaPresentation.hideSlide = function(slideIdentifier) {
        doc.getElementById(slideIdentifier).style.opacity = 0;
    };

    ninjaPresentation.currentSlide = function () {
        return win.location.hash.substring(win.location.hash.indexOf('#')+1);
    };
